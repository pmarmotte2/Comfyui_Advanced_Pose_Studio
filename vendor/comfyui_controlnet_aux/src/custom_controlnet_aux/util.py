...
import numpy as np
import torch
from huggingface_hub import constants, hf_hub_download, hf_hub_url
...
def aps_progress(message, stage="downloading_model", status="running", percent=None):
    try:
        import builtins
        callback = getattr(builtins, "ADVANCED_POSE_STUDIO_PROGRESS_CALLBACK", None)
        if callback:
            callback(message, stage=stage, status=status, percent=percent)
    except Exception:
        pass
...
def custom_hf_download(pretrained_model_or_path, filename, cache_dir=temp_dir, ckpts_dir=annotator_ckpts_path, subfolder='', use_symlinks=USE_SYMLINKS, repo_type="model"):
    ...
    if not os.path.exists(model_path):
        ...
        aps_progress(f"Downloading DWPose/OpenPose model: {filename}", stage="downloading_model")
        if use_symlinks:
        ...
        if use_symlinks:
            ...
            try:
                ...
                request = urllib.request.Request(url, headers={"User-Agent": "Advanced-Pose-Studio"})
                with urllib.request.urlopen(request, timeout=120) as response, open(part_path, "wb") as f:
                    ...
                    while True:
                        ...
                        if total:
                            ...
                            aps_progress(
                                f"Downloading DWPose/OpenPose model: {filename}",
        ...
        if not use_symlinks:
            try:
                ...
                print(e)
        aps_progress(f"Downloaded DWPose/OpenPose model: {filename}", stage="downloading_model", percent=100)
